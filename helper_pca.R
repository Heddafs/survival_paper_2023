...
#' @param principal.components PC 1-5 from PCA for all samples

model_evaluation <- function(x, y, principal.components, split.percentage = 0.67, nrep, nfolds = 3,
  ...
  results_list = lapply(1:nrep, function(i) {
    ...
    #browser()
    ## -------------Fitting PCA model (clinical and genes)----------------
    pca_data_train <- train %>%
      select(c(Survival_days, Status_survival)) %>% 
      merge(principal.components, by=0) 

    
    pca_formula <- as.formula(paste0("Surv(Survival_days, Status_survival) ~ ", paste0(colnames(principal.components), collapse = " + ")))
    model.pca <- coxph(formula = pca_formula, data = pca_data_train)
    ...
    predict_pca <- predict(model.pca, newdata = pca_data_test, type = "lp")
    Cindex_Harrell_pcamodel <- glmnet::Cindex(pred = predict_pca, y = test_y)
    Cindex_Uno_pcamodel<- survAUC::UnoC(train_y, test_y, predict_pca)
    ...
    Time_dependant_pca <- Score(object = list(Predicting_tbl$PCA_lp),
                                 formula = Surv(time, status) ~ 1,
                                 data = Predicting_tbl,
                                 times = c(183, 365, 730),
                                 metrics = 'auc')
    ROC_AUC_PCA_6m <- Time_dependant_pca$AUC$score$AUC[1]
    ROC_AUC_PCA_1y <- Time_dependant_pca$AUC$score$AUC[2]
    ROC_AUC_PCA_2y <- Time_dependant_pca$AUC$score$AUC[3]
    ...
    tibble("Iteration" = i,
           ...
           "PCA.model" = c(Cindex_Harrell_pcamodel, Cindex_Uno_pcamodel, 
                           ROC_AUC_PCA_6m, ROC_AUC_PCA_1y, ROC_AUC_PCA_2y),
      ...
      pivot_longer(cols = c(Cox.clinical, Cox.genes, Cox.both, PCA.model),
        names_to = "Model_id") %>%
